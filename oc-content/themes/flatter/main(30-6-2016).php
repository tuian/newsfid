...
//                    }
//
//                    $r = Item::newInstance()->extendData($items);
//                    $d = array_column($r, 'pk_i_id');
//                    echo '<pre>';
//                    print_r($d);
//                    print_r($items);
//                    print_r($r);
...
//                    die;
                    ...
                    <?php
                    die;
                    //osc_query_item(array("category" => $_REQUEST['filter_value']));
                    osc_query_item(array('id' => $d));
                else:
                    osc_query_item(array('category' => '', 'results_per_page' => 100));
                endif;
                ?>
                ...
                <?php else : ?>
                    ...
                    <?php while (osc_has_custom_items()) : ?>
                        ...
                        $count_query = "SELECT count(*) as total FROM `{$db_prefix}t_item_watchlist` WHERE fk_i_item_id={$item_id} GROUP BY fk_i_item_id";
                        $count_result_array = $comm->query($count_query);
                        $count_result = $count_result_array->result();
//$time = strtotime($date);
//$time = time()-$time;
//
//$years = $time/31536000;
...
//$seconds = $time;
                                    ...
                                    <div class="image">
                                        <div>
                                            <?php if (osc_count_item_resources()) { ?>
                                                <a href="<?php echo osc_item_url(); ?>"><img src="<?php echo osc_resource_preview_url(); ?>" alt="<?php echo osc_item_title(); ?>" class="img-responsive item_image" /></a>
                                            <?php } else { ?>
                                                <a href="<?php echo osc_item_url(); ?>"><img src="<?php echo osc_current_web_theme_url('images/no-image.jpg'); ?>" alt="<?php echo osc_item_title(); ?>" class="img-responsive item_image"></a>
                                            <?php } ?>
                                        </div>
                                    </div>
                                <?php } ?>
                    ...
                    <div class='paginate'>
                        <?php
                        $params = array('total' => ceil(osc_count_custom_items() / osc_default_results_per_page_at_search()), 'url' => osc_base_url(), 'class_selected' => 'selected', 'class_non_selected' => 'non_selected');
                        echo osc_pagination_items($params);
                        ?>
                    </div>
                    <?php osc_reset_custom_items() ?>
                <?php endif; ?>
            </div>
        </div>
    </div>
