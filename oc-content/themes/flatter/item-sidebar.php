            ...
            <div class="widget">
                <div class="publisher wblock clearfix">
                    <div class="pull-left avatar">
                        <a href="<?php echo osc_user_public_profile_url(osc_item_user_id()); ?>"><img  style="
                                                                                                         display: block;
                                                                                                         width: 50px;
                                                                                                         height: 50px;
                                                                                                         margin: 1em auto;
                                                                                                         ...
                                                                                                         border-radius: 99em;  "  src="<?php echo mdh_avatar_preview_url(osc_item_user_id()); ?>"></a>
                    </div>
                    <!-- Avatar -->
                    <?php if (osc_item_is_expired()) { ?>
                        <div class="not">
                            <p><i class="fa fa-info-circle pull-left warning fa-2x"></i> <?php _e("The story is expired. You can't contact the publisher.", 'flatter'); ?></p>
                        </div>
                    <?php } else { ?>
                        <div class="pull-left pub-details">
                            <?php if (osc_item_user_id() != null) { ?>
                                ...
                                <small><?php _e("Registered on", 'flatter'); ?> <?php echo osc_format_date(osc_user_regdate()); ?>

                                    <span >  <?php if (function_exists('useronline_show_user_status')) {
                            useronline_show_user_status();
                        } ?>       </span> 
...
<?php } else if (( osc_logged_user_id() == osc_item_user_id() ) && osc_logged_user_id() != 0) { ?>
                    <div class="details">
                        <div class="not">
                            <p><i class="fa fa-info-circle pull-left info fa-2x"></i> <?php _e("It's your own story, you can't contact the publisher.", 'flatter'); ?></p>
                        </div>
                    </div>
<?php } else if (osc_item_is_expired()) { ?>
    ...
    <?php } ?>
        ...
        <?php mdh_messenger_contact_form(); ?>
                                <?php } else { ?>
                                <form action="<?php echo osc_base_url(true); ?>" method="post" name="contact_form" style="display:none;" id="contact_form" <?php if (osc_item_attachment()) {
...
<?php dd_embedyoutube(); ?>
            ...
            <div class="widget">
                <h3 id="gmap"><?php _e('Story location', 'flatter'); ?></h3>
...
<?php if (osc_get_preference('position6_enable', 'flatter_theme') != '0') { ?>
                <div class="widget <?php if (osc_get_preference('position6_hide', 'flatter_theme') != '0') {
        echo"hidden-xs";
    } ?>">
