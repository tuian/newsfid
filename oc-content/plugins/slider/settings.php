...
if(Params::getParam('backgroundcolor') != '') {
...
}if(Params::getParam('text') != '') {
	$text = Params::getParam('text');
} else {
	$text = (getSliderText() != '') ? getSliderText() : '' ;
	...
	$dao_preference->update(array("s_value" => $text ), array("s_section" =>"Slider", "s_name" => "text")) ;
...
<div class="form-controls">
<select name="text">
<option <?php if($text ==1){echo 'selected="selected"';}?> value='1'><?php _e('Enable', 'Slider'); ?></option>
<option <?php if($text ==0){echo 'selected="selected"';}?> value='0'><?php _e('Disable', 'Slider'); ?></option>
</select> 
</div>
</div>
<div class="form-row">
