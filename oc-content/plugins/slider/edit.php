	...
	if( Params::getParam('update') == 1) {
		...
		if (in_array($_FILES["image"]["type"], $valid_mime_types)){
			...
			if(move_uploaded_file($_FILES["image"]["tmp_name"], $destination)){
				unlink(osc_content_path() . "plugins/slider/images/" . $slidebyid['uniqname']);
				ModelSlider::newInstance()->updateSlider($id,$uniqname,$imagename,$caption,$text,$link);
				?>
				<div class="slidersuccess">
		...
		if (copy($source, $destination )) {
			unlink(osc_content_path() . "plugins/slider/images/" . $slidebyid['uniqname']);
			ModelSlider::newInstance()->updateSlider($id,$uniqname,$actualimagename['imagename'],$caption,$text,$link);
			?>
			<div class="slidersuccess">
			<?php echo _e('The slider has been updated.','Slider'); ?>
			</div>
...
<div class="form-row">
<div class="form-label"><?php _e('Description', 'Slider'); ?></div>
<div class="form-controls"><input type="text" name="text" value="<?php echo $slidebyid['text']; ?>" placeholder="This is Description" class="xlarge"/></div>
</div>
<div class="form-row">
