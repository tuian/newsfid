	...
	if( Params::getParam('create') == 1) {
		...
		if (in_array($_FILES["image"]["type"], $valid_mime_types)){
			...
			if(move_uploaded_file($_FILES["image"]["tmp_name"], $destination)){
				ModelSlider::newInstance()->saveSlider($uniqname,$imagename,$caption,$text,$link);
				?>
		...
		if (copy($source, $destination )) {
			ModelSlider::newInstance()->saveSlider($uniqname,$actualimagename['imagename'],$caption,$text,$link);
			?>
			<div class="slidersuccess">
			<?php echo _e('The slider has been updated.','Slider'); ?>
			</div>
...
<div class="form-controls"><input type="text" name="text" value="" placeholder="This is Description" class="xlarge"/></div>
</div>
<div class="form-row">
