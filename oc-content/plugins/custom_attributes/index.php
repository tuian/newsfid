...
class CustomAttributes {
	...
	public function radio_buttons($field_id, $name, $value = null, $required = null, $search = false) {
		...
		if ($search) {
			...
			$output .= "";
			$output .= "<input type='radio' name='" . $name . "'" . $class . " value=''" . $checked . " />&nbsp;&nbsp;";
			$output .= __('Unknown<br>', CA_PLUGIN_NAME) . "" . PHP_EOL;
		}
		$options = explode(',', $options);
		foreach ($options as $option) {
			$option = trim($option);
			...
			//$output .= "<br>";
			$output .= "<input type='radio' name='" . $name . "'" . $class . " value='" . $option . "'" . $checked . " />&nbsp;&nbsp;";
	...
	public function header() {
		echo '<link href="' . osc_plugin_url(__FILE__) . 'css/ddfix.css" rel="stylesheet" type="text/css">' . PHP_EOL;
		echo '<link href="' . osc_plugin_url(__FILE__) . 'css/jquery-ui-1.10.2.custom.min.css" rel="stylesheet" type="text/css">' . PHP_EOL;
		/*$theme = osc_theme();
		if ($theme == 'bender') {
