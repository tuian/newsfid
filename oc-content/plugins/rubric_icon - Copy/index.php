...
define("PLUGIN_DIR_NAME", basename(__DIR__));
/**
 *
 */
...
function rubric_after_install() {
    $path = osc_plugin_resource(PLUGIN_DIR_NAME . '/struct.sql');
...
function rubric_after_uninstall() {
    $path = osc_plugin_resource(PLUGIN_DIR_NAME . '/drop.sql');
    $sql = file_get_contents($path);
    DAORubric::newInstance()->importSql($sql);
    @rmdir(RUBRIC_UPLOAD_DIR);
}
...
function rubric_actions() {
    ...
    if (Params::getParam('file') != 'rubric_icon/admin.php') {
        return 0;
    }
...
function upload_rubric_icon() {
    ...
    if (!empty($_FILES['imageName'])) {
        ...
        if ($myFile["error"] !== UPLOAD_ERR_OK) {
            ...
            osc_redirect_to(osc_admin_render_plugin_url(PLUGIN_DIR_NAME . '/admin.php'));
        }
...
function rubric_admin() {
    osc_admin_render_plugin('rubric_icon/admin.php');
}
...
function rubric_admin_menu() {
    osc_admin_menu_plugins('Rubric icon', osc_admin_render_plugin_url('rubric_icon/admin.php'), 'rubric_icon_submenu');
}
...
function rubric_load_styles() {
    osc_enqueue_style('rubric_icon', osc_base_url() . 'oc-content/plugins/' . PLUGIN_DIR_NAME . '/assets/css/custom_rubric_icon.css');
}
...
osc_add_hook('admin_menu_init', 'rubric_icon_menu');
// register css
