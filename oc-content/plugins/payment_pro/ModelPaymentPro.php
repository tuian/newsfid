...
class ModelPaymentPro extends DAO {
    ...
    public function getTable_wallet() {
        return DB_TABLE_PREFIX . 't_user_pack';
    }
    ...
    public function install() {
        ...
        $res = Page::newInstance()->insert(
                array('s_internal_name' => 'payment_pro_email_payment', 'b_indelible' => '1'), $description
        );
    }
...
s_extra VARCHAR( 250 ) NULL ,PRIMARY KEY(fk_s_pending_code),INDEX idx_fk_s_pending_code (fk_s_pending_code)) ENGINE=InnoDB DEFAULT CHARACTER SET 'UTF8' COLLATE 'UTF8_GENERAL_CI';", DB_TABLE_PREFIX));
        ...
        if ($version < 320) {
            ...
            foreach ($prices as $p) {
                $mp->insertPrice(
                        $p['fk_i_category_id'], (!isset($p['i_publish_cost']) || $p['i_publish_cost'] == '') ? osc_get_preference('default_publish_cost') : $p['i_publish_cost'], (!isset($p['i_premium_cost']) || $p['i_premium_cost'] == '') ? osc_get_preference('default_premium_cost') : $p['i_premium_cost'], (!isset($p['i_top_cost']) || $p['i_top_cost'] == '') ? osc_get_preference('default_top_cost') : $p['i_top_cost'], (!isset($p['i_highlight_cost']) || $p['i_highlight_cost'] == '') ? osc_get_preference('default_highlight_cost') : $p['i_highlight_cost']
                );
            }
            osc_set_preference('version', 320, 'payment_pro', 'INTEGER');
        }
    ...
    public function getPublishPrice($categoryId) {
        if (osc_get_preference('pay_per_post', 'payment_pro') == 0) {
            return 0;
        }
        $this->dao->select('*');
    ...
    public function getPremiumPrice($categoryId) {
        if (osc_get_preference('allow_premium', 'payment_pro') == 0) {
            return 0;
        }
        $this->dao->select('*');
        $this->dao->from($this->getTable_prices());
    ...
    public function getTopPrice($categoryId) {
        if (osc_get_preference('allow_top', 'payment_pro') == 0) {
            return 0;
        }
        $this->dao->select('*');
        $this->dao->from($this->getTable_prices());
    ...
    public function getHighlightPrice($categoryId) {
        if (osc_get_preference('allow_highlight', 'payment_pro') == 0) {
            return 0;
        }
        $this->dao->select('*');
        $this->dao->from($this->getTable_prices());
    ...
    public function getWallet($userId) {
        ...
        $this->dao->where('user_id', $userId);
        $result = $this->dao->get();
        if ($result) {
            $row = $result->row();
            $row['premium_post'] = (isset($row['premium_post']) ? $row['premium_post'] : 0);
            $row['remaining_post'] = (isset($row['remaining_post']) ? $row['remaining_post'] : 0);
            return $row;
        }
        return false;
    }
    ...
    public function insertPrice($category, $publish_fee, $premium_fee, $top_fee, $highlight_fee, $tax = 0) {
        return $this->dao->replace(
                        $this->getTable_prices(), array(
    ...
    public function payPremiumFee($itemId, $invoiceId) {
        if (strtoupper($invoiceId) == "ADMIN") {
            $invoiceId = $this->adminInvoice(
                    array(
                        'description' => sprintf(__('Premium enchancement for listing %d', 'payment_pro'), $itemId),
                        'item_id' => $itemId,
                        'id' => 'PRM00-' . $itemId
                    )
            );
    ...
    public function payHighlightFee($itemId, $invoiceId) {
        if (strtoupper($invoiceId) == "ADMIN") {
            $invoiceId = $this->adminInvoice(
                    array(
                        'description' => sprintf(__('Highlight enchancement for listing %d', 'payment_pro'), $itemId),
                        'item_id' => $itemId,
                        'id' => 'HLT-' . $itemId
                    )
            );
    ...
    public function addQueue($date, $item, $publish = true, $premium = true) {
        return $this->dao->insert(
                        $this->getTable_mail_queue(), array(
                    'dt_send_date' => $date,
                    'fk_i_item_id' => $item,
                    'b_publish' => $publish,
                    'b_premium' => $premium
    ...
    public function adminInvoice($params = null) {
        ...
        return $this->saveInvoice(
                        "ADMIN_" . osc_logged_admin_id(), $rows['amount'], $rows['amount_tax'], $rows['amount_total'], PAYMENT_PRO_COMPLETED, osc_get_preference('currency', 'payment_pro'), osc_logged_admin_email(), osc_logged_admin_id(), "ADMIN", array($rows)
        );
    }
