...
s_extra VARCHAR( 250 ) NULL ,PRIMARY KEY(fk_s_pending_code),INDEX idx_fk_s_pending_code (fk_s_pending_code)) ENGINE=InnoDB DEFAULT CHARACTER SET 'UTF8' COLLATE 'UTF8_GENERAL_CI';", DB_TABLE_PREFIX));
            ...
            if($version < 320 ) {
                ...
                foreach($prices as $p) {
                    $mp->insertPrice(
                        $p['fk_i_category_id'],
                        (!isset($p['i_publish_cost']) || $p['i_publish_cost']=='')?osc_get_preference('default_publish_cost'):$p['i_publish_cost'],
                        (!isset($p['i_premium_cost']) || $p['i_premium_cost']=='')?osc_get_preference('default_premium_cost'):$p['i_premium_cost'],
                        (!isset($p['i_top_cost']) || $p['i_top_cost']=='')?osc_get_preference('default_top_cost'):$p['i_top_cost'],
                        (!isset($p['i_highlight_cost']) || $p['i_highlight_cost']=='')?osc_get_preference('default_highlight_cost'):$p['i_highlight_cost']
                    );
                }
                osc_set_preference('version', 320, 'payment_pro', 'INTEGER');
            }
