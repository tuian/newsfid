...
if(Params::getParam('plugin_action')=='add_category') {
    if(Params::getParam('categoryId')!='') {
        ModelPaymentPro::newInstance()->insertPrice(
            Params::getParam('categoryId'),
...
?>
<style type="text/css">
    .payment-pro-pub, .payment-pro-prm, .payment-pro-top, .payment-pro-hlt {
...
<script type="text/javascript" >
    ...
    function edit_cat(id, pub, prm, top, hlt) {
                    ...
                    <form name="payment_form" id="payment_form" action="<?php echo osc_admin_base_url(true);?>" method="POST" enctype="multipart/form-data" >
                        ...
                        <table border="0" cellpadding="0" cellspacing="0">
                            <tr>
                                ...
                                <td style="width:175px;"><?php echo sprintf(__('Move to top fee (%s)', 'payment_pro'), osc_get_preference('currency', 'payment_pro')); ?></td>
                                <td style="width:175px;"><?php echo sprintf(__('Highlight fee (%s)', 'payment_pro'), osc_get_preference('currency', 'payment_pro')); ?></td>
                            </tr>
                        ...
                        <?php foreach($prices as $price) {
                            ...
                            <div class="form-row">
                                ...
                                <div class="form-controls">
                                    <span class="payment-pro-pub" ><?php printf(__('Publish cost: %s'), osc_format_price($price['i_publish_cost'], osc_get_preference('currency', 'payment_pro'))); ?></span>
                                    <span class="payment-pro-prm" ><?php printf(__('Premium cost: %s'), osc_format_price($price['i_premium_cost'], osc_get_preference('currency', 'payment_pro'))); ?></span>
                                    <span class="payment-pro-top" ><?php printf(__('Move to top cost: %s'), osc_format_price($price['i_top_cost'], osc_get_preference('currency', 'payment_pro'))); ?></span>
                                    <span class="payment-pro-hlt" ><?php printf(__('Highlight cost: %s'), osc_format_price($price['i_highlight_cost'], osc_get_preference('currency', 'payment_pro'))); ?></span>
                                    <span><a href="javascript:edit_cat(<?php echo $price['fk_i_category_id'].", ".($price['i_publish_cost']/1000000).", ".($price['i_premium_cost']/1000000).", ".($price['i_top_cost']/1000000).", ".($price['i_highlight_cost']/1000000); ?>);" ><?php _e('edit', 'payment_pro'); ?></a></span>
...
<form id="dialog-new" method="post" action="<?php echo osc_admin_base_url(true); ?>" class="has-form-actions hide">
    ...
    <div class="form-horizontal">
        ...
        <div class="form-row">
            ...
            <div class="form-controls"><input type="text" id="top_price" name="top_price" value="" placeholder="<?php echo osc_get_preference('default_top_cost', 'payment_pro'); ?>" /> <?php echo osc_get_preference('currency',
                    'payment_pro'); ?></div>
        </div>
        <div class="form-row">
            <div class="form-label"><?php _e('Highlight price', 'payment_pro'); ?></div>
            <div class="form-controls"><input type="text" id="highlight_price" name="highlight_price" value="" placeholder="<?php echo osc_get_preference('default_highlight_cost', 'payment_pro'); ?>" /> <?php echo osc_get_preference('currency',
                    'payment_pro'); ?></div>
        </div>
