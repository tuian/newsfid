...
*/
                ...
                if(Params::getParam('payment_pro_make_highlight')==1) {
                    if($item['b_enabled']==1) {
                        payment_pro_cart_add(
                            ...
                            sprintf(__('Highlight enchancement for listing %d', 'payment_pro'), $item['pk_i_id']),
                            $highlight_fee['price'],
                            1,
    ...
    function payment_pro_admin_page_header() {
        ...
        switch (Params::getParam('route')) {
            ...
            case 'payment-pro-admin-subs':
                osc_remove_hook('admin_page_header', 'customPageHeader');
                osc_add_hook('admin_page_header',  'payments_pro_admin_page_header_subs');
                break;
            case 'payment-pro-admin-wallet':
                osc_remove_hook('admin_page_header', 'customPageHeader');
                osc_add_hook('admin_page_header',  'payments_pro_admin_page_header_wallet');
                break;
            case 'payment-pro-admin-packs':
                osc_remove_hook('admin_page_header', 'customPageHeader');
                osc_add_hook('admin_page_header',  'payments_pro_admin_page_header_packs');
                break;
            default:
                break;
        }
    ...
    function payments_pro_admin_page_header_log() {
        ?>
        <h1><?php _e('History of payments', 'payment_pro'); ?></h1>
        <?php @include(dirname(__FILE__) . '/admin/header.php');
    }

    function payments_pro_admin_page_header_subs() {
        ?>
        <h1><?php _e('Subscriptions', 'payment_pro'); ?></h1>
        <?php @include(dirname(__FILE__) . '/admin/header.php');
    }

    function payments_pro_admin_page_header_wallet() {
        ?>
        <h1><?php _e('Add credit to users', 'payment_pro'); ?></h1>
        <?php @include(dirname(__FILE__) . '/admin/header.php');
    }

    function payments_pro_admin_page_header_packs() {
        ?>
        <h1><?php _e('Manage your credit packs', 'payment_pro'); ?></h1>
        <?php @include(dirname(__FILE__) . '/admin/header.php');
    }
    ...
    function payment_pro_admin_title() {
        switch (Params::getParam('route')) {
            ...
            case 'payment-pro-admin-subs':
                osc_remove_filter('admin_title', 'customPageTitle');
                osc_add_filter('admin_title',  'payments_pro_admin_title_subs' );
                break;
            case 'payment-pro-admin-wallet':
                osc_remove_filter('admin_title', 'customPageTitle');
                osc_add_filter('admin_title',  'payments_pro_admin_title_wallet' );
                break;
            case 'payment-pro-admin-packs':
                osc_remove_filter('admin_title', 'customPageTitle');
                osc_add_filter('admin_title',  'payments_pro_admin_title_packs' );
                break;
            default:
                break;
        }
    ...
    function payments_pro_admin_title_settings($string) {
        return __('Settings, Osclass payments pro', 'payment_pro').$string;
    }
    function payments_pro_admin_title_categories_fees($string) {
        return __('Categories fees, Osclass payments pro', 'payment_pro').$string;
    }
    function payments_pro_admin_title_log($string) {
        return __('History of payments, Osclass payments pro', 'payment_pro').$string;
    }
    function payments_pro_admin_title_subs($string) {
        return __('Subscriptions, Osclass payments pro', 'payment_pro').$string;
    }
    function payments_pro_admin_title_wallet($string) {
        return __('Add credit to users, Osclass payments pro', 'payment_pro').$string;
    }
    ...
    function payment_pro_item_detail($item) {
        if(osc_is_web_user_logged_in() && isset($item['pk_i_id']) && ModelPaymentPro::newInstance()->isEnabled($item['pk_i_id']) && isset($item['fk_i_user_id']) && $item['fk_i_user_id']==osc_logged_user_id()) {
            ...
            if (osc_get_preference("allow_premium", 'payment_pro') == "1") {
                if (ModelPaymentPro::newInstance()->premiumFeeIsPaid($item['pk_i_id'])) {
            ...
            if (osc_get_preference("allow_highlight", 'payment_pro') == "1") {
                if (ModelPaymentPro::newInstance()->highlightFeeIsPaid($item['pk_i_id'])) {
